...
#include "asterisk/speech_sphinx.h"
...
/*! \brief API description */
	 static struct ast_speech_engine SPHINX_ENGINE_INFO = { "Sphinx",
		 sphinx_create,
		 sphinx_destroy,
		 sphinx_load,
		 sphinx_unload,
		 ...
		 sphinx_start,
		 sphinx_change,
		 sphinx_get_setting, /* Added by jcli.china@gmail.com, 2014.04.16 */		 
		 sphinx_change_results_type,
		 sphinx_get,
		 NULL, /* Added by jcli.china@gmail.com, 2014.04.16 */
		 {NULL} /* Added by jcli.china@gmail.com, 2014.04.16 */
	 };

/*! \brief Global settings */
char SPHINX_SERVER_ADDR[] = "127.000.000.001"; /* Sloppy? */
int SPHINX_SERVER_PORT = 0;
...
static int load_module(void)
{
	...
	struct ast_config *conf = ast_config_load("sphinx.conf", config_flags);
	char const *value;
	/* Added by jcli.china@gmail.com, 2014.04.16 */
	struct ast_format_cap *formats = NULL;
	struct ast_format format = {AST_FORMAT_SLINEAR, {{0}, 0}};	
	/* Ended */
	...
	/* Added by jcli.china@gmail.com, 2014.04.16 */
	formats = ast_format_cap_alloc(AST_FORMAT_CAP_FLAG_NOLOCK);
	ast_format_cap_add(formats, &format);
	SPHINX_ENGINE_INFO.formats = formats;
	/* Ended */
...
int sphinx_create(struct ast_speech *speech, struct ast_format *format)
{
...
int sphinx_load(struct ast_speech *speech, const char *grammar_name, const char *grammar)
{
...
int sphinx_unload(struct ast_speech *speech, const char *grammar_name)
{
...
int sphinx_activate(struct ast_speech *speech, const char *grammar_name)
{
	struct sphinx_request sr;
	sr.rtype = REQTYPE_GRAMMAR;
	sr.dlen = strlen(grammar_name) + 1;
	sr.data = (char*)grammar_name;
...
int sphinx_deactivate(struct ast_speech *speech, const char *grammar_name)
{
...
int sphinx_change(struct ast_speech *speech, const char *name, const char *value)
{
...
int sphinx_get_setting(struct ast_speech *speech, const char *name, char *buf, size_t len)
{
	return -1;
}
